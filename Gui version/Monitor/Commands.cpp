...
{
    command = "nvidia-settings -t -q gpus 2>/dev/null | sed -e 's/^ *//'";
}
void Commands::getGpuAllStates(std::string &command, std::string gpuid)
{
    command = "nvidia-settings -t -q " + gpuid + "/GPUCoreTemp -q " + gpuid + "/TotalDedicatedGPUMemory -q " + gpuid + "/UsedDedicatedGPUMemory 2>/dev/null";
}
void Commands::getGpuUtilizations(std::string &command)
{
    command = "nvidia-smi --query-gpu=utilization.memory,utilization.gpu --format=csv,nounits,noheader 2>/dev/null";
}
void Commands::getGpuClocks(std::string &command)
{
    command = "nvidia-smi --query-gpu=clocks.mem,clocks.gr --format=csv,nounits,noheader 2>/dev/null";
}

void Commands::getCpuProcessorUtilization(std::string &command)
{
    command = "top 1 -b -i -n 1 2>/dev/null | grep Cpu | tr ',' '.'";
}
void Commands::getCpuCoreTemp(std::string &command, int coreamount)
{
    std::stringstream sstream;
    sstream << "cat /sys/bus/platform/devices/coretemp.0/hwmon/hwmon*/temp[2-" << coreamount << "]_input 2>/dev/null";
    command = sstream.str();
}
void Commands::getCpuState(std::string &command)
{
    command = "cat /proc/cpuinfo 2>/dev/null | grep -P 'cache size|cpu MHz|cpu cores|model name'";
}
...
bool Commands::loadGpuUtilizations(std::vector<std::string> *vector)
{
    std::size_t size = vector->size();
    std::string command;
    getGpuUtilizations(command);
    FILE *in;
    char buff[512];
...
void Commands::parseGpuString(std::vector<std::string> *vector)
{
    std::string rawstring;
    std::string memclock;
